...
wordz = 12345
def reverse_characters_or_numbers(wordz):
    if type(wordz) == int: 
        wordz = str(wordz)
        wordz = list(wordz)
        wordz.reverse()
        ...
        wordz = ''.join(wordz)
    return wordz
print(reverse_characters_or_numbers(wordz))
...
def list_function(listz):
    list_new = []
    for i in listz:
        reverse_item = reverse_characters_or_numbers(i)
        list_new.append(reverse_item)
    list_new.reverse()
    return list_new


print(list_function(list_test3))
